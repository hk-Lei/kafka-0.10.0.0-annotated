...
class WorkerSourceTask extends WorkerTask {
    ...
    // outstandingMessages 的大小会累加，当其大小到一定值时，此线程等待一会，
    // 使 outstandingMessages 被删除一些
    ...
    public void execute() {
        try {
            ...
            while (!isStopping()) {
                ...
                // 当 outstandingMessages 的大小累计到临界值时，线程等待一会,
                while (outstandingMessages.size() > OUTSTANDING_MESSAGES_THRESHOLD) {
                    log.info("About {} outstandingMessages records to Kafka, stopRequestedLatch.await({}ms)",
                            outstandingMessages.size(), SEND_WAIT_MS);
