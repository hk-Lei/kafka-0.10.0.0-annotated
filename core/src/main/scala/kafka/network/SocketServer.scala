...
class SocketServer(val config: KafkaConfig, val metrics: Metrics, val time: Time) extends Logging with KafkaMetricsGroup {
  ...
  // TRACE: Currently identical to PLAINTEXT and used for testing only. We may implement extra instrumentation when testing channel code. */
  private val endpoints = config.listeners
  // num.network.threads
  private val numProcessorThreads = config.numNetworkThreads
  // queued.max.requests
  private val maxQueuedRequests = config.queuedMaxRequests
  // 总的线程数是 配置的 endpoints 的数 X num.network.threads
  private val totalProcessorThreads = numProcessorThreads * endpoints.size

  // 每个 IP 的最大连接数 max.connections.per.ip
  ...
  // 包含一个请求队列 和一组响应队列
  // 请求队列数 ArrayBlockingQueue 实例，size 为 queued.max.requests 配置大小
  // 响应队列是 LinkedBlockingQueue 实例一组数组，数组大小为 totalProcessorThreads
  ...
  def startup() {
    this.synchronized {
      ...
      // socket.send.buffer.bytes 配置发送 buffer 的大小
      val sendBufferSize = config.socketSendBufferBytes
      // socket.receive.buffer.bytes 配置接受 buffer 的大小
      val recvBufferSize = config.socketReceiveBufferBytes
      val brokerId = config.brokerId

      var processorBeginIndex = 0
      // 每个 endpoint 一个 acceptor 线程 例如： kafka-socket-acceptor-PLAINTEXT-9092
      // 每个 Acceptor 管理着 numProcessorThreads （即 num.network.threads） 个处理线程
      // 使用 round robin 轮询调度
      endpoints.values.foreach { endpoint =>
        ...
        // 使用 CountDownLatch 阻塞当前线程等待 Acceptor 启动完成
