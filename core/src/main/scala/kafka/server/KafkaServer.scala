...
class KafkaServer(val config: KafkaConfig, time: Time = SystemTime, threadNamePrefix: Option[String] = None) extends Logging with KafkaMetricsGroup {
  ...
  def startup() {
    try {
      ...
      // 判断 Server 状态
      // 1 关闭状态： throw IllegalStateException
      // 2 已启动状态 ：直接 return
      ...
      if (canStartup) {
        ...
        /* start scheduler */
        // 每个 Server 启动一个 ScheduledThreadPoolExecutor 服务
        // 其运行的线程数通过 background.threads 配置项配置
        kafkaScheduler.startup()

        /* setup zookeeper */
        // 创建 zk 连接
        // 创建 chroot znode 并创建基于 chroot 的其他 znode
        zkUtils = initZk()

        /* start log manager */
        // 创建 logManager, 加载个 tp 的 checkpoint
        logManager = createLogManager(zkUtils.zkClient, brokerState)

        /**
          * 注册到 Scheduler 的服务
          * 1. kafka-log-retention 线程 ：检测 kafka 数据目录的时间和大小，定期删除过期数据
          * 2. kafka-log-flusher 线程 ：定期将 log segment flush 到磁盘
          * 3. kafka-recovery-point-checkpoint 线程 ：定期将 checkpoint 刷新到 recovery-point-offset-checkpoint 文件中
          */
        logManager.startup()
        ...
        // 创建并启动 SocketServer
        // 包含处理请求和响应的队列， Acceptor 线程及一批 Processor 线程
  ...
  private def createLogManager(zkClient: ZkClient, brokerState: BrokerState): LogManager = {
    ...
    // 获取 zk 中存储的 topic 的配置项添加到 default 配置项中
    ...
    // 创建 log.dirs 或者 log.dir 的配置目录，（创建失败就 throw 异常，建议提前创建），
    // 在配置数据目录中创建 .lock 文件锁该目录
